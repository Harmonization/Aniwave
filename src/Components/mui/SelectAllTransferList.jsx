...
export default function SelectAllTransferList({url, openImg, setOpen, diskFiles}) {
  ...
  const handleCheckedRight = async () => {
    ...
    await postFetch(url, 'download_files', {paths: leftChecked})
  ...
  const customList = (title, items) => (
    <Card sx={{backgroundColor: 'rgba(0,0,0,.01)'}}>
      <CardHeader
        sx={{ px: 2, py: 1 }}
        avatar={
          <Checkbox
  ...
  return (
    <Grid container spacing={2} justifyContent="center" alignItems="center">
      <Grid item>{customList('На облаке', left)}</Grid>
      <Grid item>
      ...
      <Grid item>{customList('Скачано', right)}</Grid>
    </Grid>
  );
}
